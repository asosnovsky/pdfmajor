...
def process_command_stream(
    streams: List[PDFStream], font_cache: dict = None, state: PDFStateStack = None
):
                ...
                if nargs:
                    ...
                    if len(args) == nargs:
                        ...
                        raise CommandProcessorError(
                            f"Invalid Number of Args provided {method}"
                        )
                else:
                    func(state)
            else:
        ...
        for complete_item in state.complete_layout_items:
            if isinstance(complete_item, LTXObject):
                ...
                for item in process_command_stream(
                    [complete_item.stream], font_cache=font_cache, state=xobj_state
                ):
