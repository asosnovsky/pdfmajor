...
def convert_file(
    ...
    if output_file is None:
        output_file = os.path.join(
            ...
            os.path.basename(re.sub(r"\.\w+$", "." + out_type, input_file)),
        )
    if re.search(re.escape(out_type) + r"$", output_file) is None:
        raise ConverterException(
            "Please make sure that the file name and output type match!"
        )
    if out_type == "html":
        ...
        raise ConverterException(
            "Please specify out_type as 'html' or 'xml' or 'json' or 'text' or 'yaml'"
        )
