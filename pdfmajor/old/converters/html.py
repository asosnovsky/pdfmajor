...
def render_page(html: HTMLMaker, ltpage: PageInterpreter):
    ...
    with html.elm(
        "div",
        {"class": "page", "id": f"page-{ltpage.page_num}"},
        {"width": f"{ltpage.width}px", "height": f"{ltpage.height}px"},
    ):
        for child in ltpage:
            render(child)
...
def render_curve(html: HTMLMaker, ltpage: PageInterpreter, item: LTCurve):
    ...
    with html.elm(
        ...
        html.singleton("path", {"d": path, **path_css}, {**path_css})


def create_svg_path(item: LTCurve):
    path = ""
    svg_attr = {"class": "curve", "z-index": 2}

    if item.height > 0:
        dy = 1.0 / item.height
        dx = (item.width / item.height) / max([(item.x1 - item.x0), 0.01])
        svg_attr.update({"viewBox": "0 0 %s %s" % (item.width / item.height, 1)})
    else:
        dx = 1.0 / (item.x1 - item.x0)
        dy = 1.0
        svg_attr.update({"viewBox": "0 0 1 1"})
