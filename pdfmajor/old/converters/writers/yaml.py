...
class YAMLMakerWriter:
    def __init__(
        self, outfile: TextIOWrapper, codec: str = "utf-8", levels_deep: int = -1
    ):
    ...
    def __write_raw(self, text):
        if self.outfile is not None:
            ...
            raise FileAccessException(
                "Attempting to write to file without an open connection"
            )

    def _write(
        self,
        text: str,
        append_deep: bool = True,
        deep_space: str = "  ",
        prefix: str = "",
    ):
        if not append_deep:
...
class YAMLMakerObject(YAMLMakerWriter):
    ...
    def write(self, key: str, value: str):
        if key not in self.keys:
            ...
            if len(self.keys) > 1:
                ...
                self._write(
                    f"{key}: {value}", prefix="", append_deep=not self.parent_is_array
                )
    ...
    def object(self, key: str):
        ...
        with YAMLMakerObject(
            self.outfile, codec=self.codec, levels_deep=self.levels_deep
        ) as obj:
            yield obj
