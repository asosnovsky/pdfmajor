...
def save_state(stack: PDFStateStack) -> PDFStateStack:
    stack.gstack.append([stack.t_matrix, stack.text.copy(), stack.graphics.copy()])
    return stack
...
# concat-matrix
@PDFCommands.add("cm")
def concat_transition_matrix(
    stack: PDFStateStack, a1, b1, c1, d1, e1, f1
) -> PDFStateStack:
...
def curve_lineto(stack: PDFStateStack, x: float, y: float) -> PDFStateStack:
    stack.curvestacks.append(
        CurvePath(CurvePath.METHOD.LINE_TO, CurvePoint(stack.t_matrix, x, y))
    )
    return stack
...
@PDFCommands.add("v")
def curve_urveto(
    stack: PDFStateStack, x2: float, y2: float, x3: float, y3: float
) -> PDFStateStack:
    stack.curvestacks.append(
        CurvePath(
...
@PDFCommands.add("y")
def curve_rveto(
    stack: PDFStateStack, x1: float, y1: float, x3: float, y3: float
) -> PDFStateStack:
    stack.curvestacks.append(
        CurvePath(
...
def curve_rect(stack: PDFStateStack, x, y, w, h) -> PDFStateStack:
    stack.curvestacks.extend(
        [
            ...
            CurvePath(
                CurvePath.METHOD.LINE_TO, CurvePoint(stack.t_matrix, x + w, y + h)
            ),
...
def curve_complete_path(stack: PDFStateStack) -> PDFStateStack:
    stack.complete_layout_items.append(
        make_curve(stack.t_matrix, stack.graphics.copy(), False, stack.curvestacks)
    )
    stack.curvestacks = []
    return stack
...
def curve_complete_path_evenodd(stack: PDFStateStack) -> PDFStateStack:
    stack.complete_layout_items.append(
        make_curve(stack.t_matrix, stack.graphics.copy(), True, stack.curvestacks)
    )
    stack.curvestacks = []
    return stack
...
def set_stroke_gray(stack: PDFStateStack, gray: float) -> PDFStateStack:
    stack.graphics.set_stroke_color(stack.colorspace_map["DeviceGray"], gray)
    return stack
...
def set_nonstroke_gray(stack: PDFStateStack, gray: float) -> PDFStateStack:
    stack.graphics.set_nostroke_color(stack.colorspace_map["DeviceGray"], gray)
    return stack
...
def set_stroke_cmyk(stack: PDFStateStack, c, m, y, k) -> PDFStateStack:
    stack.graphics.set_stroke_color(stack.colorspace_map["DeviceCMYK"], c, m, y, k)
    return stack
...
def set_stroke_colorspace_custom(stack: PDFStateStack) -> PDFStateStack:
    ...
    stack.graphics.set_stroke_color(stack.graphics.scolspace, *components)
    return stack
...
def nextline(stack: PDFStateStack) -> PDFStateStack:
    ...
    stack.text.matrix = (
        ...
        c,
        d,
        stack.text.leading * c + e,
        stack.text.leading * d + f,
    )
    stack.text.linematrix = [0, 0]
    return stack
...
def add_char_blocks(stack: PDFStateStack, seq: bytearray) -> PDFStateStack:
    ...
    stack.current_textblock.add_char_block(
        seq, stack.t_matrix, stack.text.copy(), stack.graphics.ncolor.copy()
    )
    return stack
...
# doublequote
@PDFCommands.add("_w")
def add_dbquoted_char_blocks(
    stack: PDFStateStack, wordspace, charspace, b
) -> PDFStateStack:
    stack.text.wordspace = wordspace
    stack.text.charspace = charspace
    return add_char_blocks(stack, [b])
...
def insert_image(stack: PDFStateStack, obj) -> PDFStateStack:
    if "W" in obj and "H" in obj:
        stack.complete_layout_items.append(make_image(obj, stack.t_matrix))
    return stack
