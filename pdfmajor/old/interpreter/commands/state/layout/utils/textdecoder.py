...
def decode_text_seq(
    seq: bytearray, ctm: tuple, textstate: PDFTextState, color: PDFColor
):
    ...
    if textstate.font.is_vertical():
        return __render_string_along(1, seq, textstate, dxscale, color)
    else:
        return __render_string_along(0, seq, textstate, dxscale, color)


def __render_string_along(
    idx: int, seq: bytearray, textstate: PDFTextState, dxscale: float, color: PDFColor
):
    needcharspace = False
    char_meta_datas = []
    for obj in seq:
        if isnumber(obj):
            ...
            for cid in textstate.font.decode(obj):
                ...
                char_meta_datas.append((text, bbox))
