...
import logging
import sys
from pdfmajor.converters import convert_file  # type: ignore
...
def make_argparser():
    ...
    parser.add_argument(
        "-P", "--password", type=str, default=None, help="Decryption password for PDF"
    )
    parser.add_argument(
...
def main(args=None):
    ...
    if len(parsed_args.files) == 1:
        ...
        convert_file(parsed_args.files[0], **func_args)
    else:
